...
def train_and_log(train_config, model_config, training_dataset=None, model=None, wandb_enabled=False, criterion=None, multiclass=False, wandb_project=None):
    ...
    train_loss, train_accuracy = train_lstm(model, train_loader, criterion, optimizer, train_config["epochs"])
    if wandb_enabled:
        if wandb_project is None:
            project_name = "SL_pipeline"
        else:
...
def evaluate_and_log(config, validation_dataset=None, model=None, criterion=None, multiclass=False, wandb_project=None):
    ...
    return evaluate_lstm(model, val_loader, criterion=criterion)

#%% Pipeline MULTIMODAL

def pipeline_lstm(dataset_name, label_name, num_classes):
...
def pipeline_EMG_lstm(dataset_name, label_name, num_classes):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=train_config_EMG['learning_rate'])
...
trained_model_lstm = pipeline_lstm("prova_senza5_dataset.npy", "prova_senza5_labels.npy", 6)
model_path = 'model_params_lstm.pth'
torch.save(trained_model_lstm.state_dict(), model_path)
trained_model_emg_lstm = pipeline_EMG_lstm("prova_senza5_dataset.npy", "prova_senza5_labels.npy", 6)
model_path = 'model_params_lstm_emg.pth'
torch.save(trained_model_emg_lstm.state_dict(), model_path)
trained_model_raw_imu_lstm = pipeline_raw_IMU_lstm("prova_senza5_dataset.npy", "prova_senza5_labels.npy", 6)
model_path = 'model_params_lstm_raw_imu.pth'
torch.save(trained_model_raw_imu_lstm.state_dict(), model_path)
...
emg_angles, imu_angles, label_angles, pred1, pred2, pred_lstm_angles = (data['emg'], data['imu'], data['label'], data['prediction_0'], data['prediction_1'], data['prediction_2'])
pred_lstm_angles = pred_lstm_angles.squeeze()
data = np.load(file_path_raw_imu, allow_pickle=True)
emg_raw_imu, imu_raw_imu, label_raw_imu, pred1, pred2, pred_lstm_raw_imu = (data['emg'], data['imu'], data['label'], data['prediction_0'], data['prediction_1'], data['prediction_2'])
pred_lstm_raw_imu = pred_lstm_raw_imu.squeeze()
data = np.load(file_path_emg, allow_pickle=True)
emg_emg, imu_emg, label_emg, pred1, pred2, pred_lstm_emg = (data['emg'], data['imu'], data['label'], data['prediction_0'], data['prediction_1'], data['prediction_2'])
pred_lstm_emg = pred_lstm_emg.squeeze()
...
model_angles_from_online_lstm = pipeline_lstm_from_online(emg_angles, imu_angles, label_angles, num_classes=5, model_path=BASE_MODEL_PATH+'/models/model_lstm.pth')
...
model_raw_imu_from_online_lstm = pipeline_raw_IMU_lstm_from_online(emg_raw_imu, imu_raw_imu, label_raw_imu, num_classes=5, model_path=BASE_MODEL_PATH+'/models/model_lstm_raw_imu.pth')

emg_emg, imu_emg, label_emg = undersample_majority_class_first_n(emg_data=emg_emg, imu_data=imu_emg, labels=label_emg)
model_emg_from_online_lstm = pipeline_EMG_lstm_from_online(emg_emg, imu_emg, label_emg, num_classes=5, model_path=BASE_MODEL_PATH+'/models/model_lstm_emg.pth')


#%% SAVE MODELS

model_path_lstm = '//wsl.localhost/Ubuntu/home/dema/ros2_ws/models/model_lstm.pth'
model_path_lstm_raw_imu = '//wsl.localhost/Ubuntu/home/dema/ros2_ws/models/model_lstm_raw_imu.pth'
model_path_lstm_emg = '//wsl.localhost/Ubuntu/home/dema/ros2_ws/models/model_lstm_emg.pth'
torch.save(model_angles_from_online_lstm.state_dict(), model_path_lstm)
...
model_angles_from_online_lstm = pipeline_lstm_from_online(data['emg_angles'], data['imu_angles'], data['label_angles'], num_classes=5, model_path=None, save=True, model_path_save="models/model_lstm9.pth")#, save=True)
...
model_raw_imu_from_online_lstm = pipeline_raw_IMU_lstm_from_online(data['emg_raw_imu'], data['imu_raw_imu'], data['label_raw_imu'], num_classes=5, model_path=None, save=True, model_path_save="models/model_lstm_raw_imu9.pth")#, save=True)

data['emg_emg'], _, data['label_emg'] = undersample_majority_class_first_n(emg_data=data['emg_emg'], labels=data['label_emg'])
model_emg_from_online_lstm = pipeline_EMG_lstm_from_online(data['emg_emg'], None, data['label_emg'], num_classes=5, model_path=None, save=True, model_path_save="models/model_lstm_emg.pth")#, save=True)
...
for participant_folder in participant_folders:
    ...
    if data['emg_angles'].shape[1] == 11:
        ...
        data['emg_raw_imu'] = np.delete(data['emg_raw_imu'], [0, 6], axis=1)
        data['emg_emg'] = np.delete(data['emg_emg'], [0, 6], axis=1)
    angles_model = pipeline_lstm_from_online(data['emg_angles'], data['imu_angles'], data['label_angles'], num_classes=5, model_path=None, save=False, model_path_save = "model_ffnn_angles.pth", participant_folder=participant_folder)
    raw_imu_model = pipeline_raw_IMU_lstm_from_online(data['emg_raw_imu'], data['imu_raw_imu'], data['label_raw_imu'], num_classes=5, model_path=None, save=False, model_path_save = "model_ffnn_raw_imu.pth", participant_folder=participant_folder)
    emg_model = pipeline_EMG_lstm_from_online(data['emg_emg'], data['label_emg'], num_classes=5, model_path=None, save=False, model_path_save = "model_ffnn_emg.pth", participant_folder=participant_folder)
...
for fold, (train_idx, test_idx) in enumerate(kf.split(dataset)):
    ...
    config = {
        ...
        #'hidden_sizes_emg': [256, 512, 256],
        #'hidden_sizes_imu': [256, 512, 256],
        'input_shape_emg': 9 * 4,#(num_emg_channels, 4),
        'input_shape_imu': 9,
        'dropout_rate': 0.1
    }
    # Initialize the model
