...
class DataManagerEmg:
    ...
    def append_data(self, emg, imu, label, prediction1, prediction2, prediction3):
        ...
        # only here the imu is different
        batch_results = np.array([
            emg.cpu().numpy(),
            np.array([0]),#imu.cpu().numpy() if not isinstance(imu, str) else imu,
            label.cpu().numpy(),
            prediction1,
            prediction2,
            prediction3
        ...
        print(f"len of the buffer emg: {len(self.buffer)}")

    def save_data(self, directory=None):
        if directory:
            os.makedirs(directory, exist_ok=True)
        
        file_name = "emg_dataset.npz"
        ...
        #np.savetxt(os.path.join(directory, 'recording_time_emg.txt'), np.array([self.recording_time_start]), fmt='%f')
        self.reset_buffer()
...
class PipelineNodeEmg(Node):
    def __init__(self):
        super().__init__('pipeline_node_emg')
        ...
        self.subscription = self.create_subscription(RecordingFolder, '/folder_info_emg', self.participant_info_callback, qos_profile=rclpy.qos.qos_profile_sensor_data, callback_group=ReentrantCallbackGroup())
        ...
        self.data_manager = DataManagerEmg()
        # Neural Network
        self.config = {
        ...
        self.models = [self.model_ffnn_emg, self.model_lstn_emg, self.model_cnn_emg] = [None] * 3
        self.models_type = ['ffnn', 'lstm', 'cnn']
        self.config['input_type'] = 'emg'
    ...
    def training_path_callback(self, msg):
        ...
        parts[-1] = "DATASET_EMG_" + last_char
        self.training_path = '/'.join(parts)
        self.training_pipeline_emg()
    ...
    def training_pipeline(self):
        ...
        emg_data, imu_data, label_data, prediction1, prediction2, prediction3 = self.data_manager.load_data('emg_dataset.npz', directory=self.training_path)
