...
class DataManager:
    ...
    def save_data(self, directory=None):
        ...
        file_name = "angles_estimation_dataset.npz"
        ...
        np.savetxt(os.path.join(directory, 'rec_time_matlab_raw_signal.txt'), np.array([self.recording_time_start]), fmt='%f')
        self.reset_buffer()
...
class PipelineNode(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(RecordingFolder, '/folder_info', self.participant_info_callback, qos_profile=rclpy.qos.qos_profile_sensor_data, callback_group=ReentrantCallbackGroup())
        ...
        self.models = [self.model_ffnn_angles, self.model_lstn_angles, self.model_cnn_angles] = [None] * 3
        self.models_type = ['ffnn', 'lstm', 'cnn']
        self.config['input_type'] = 'emg+angles'
    ...
    def create_and_load_model(self, return_config=False):
        ...
        if self.config['model_type'] == 'ffnn':
            if self.config['input_type'] == 'emg+raw_imu':
                ...
                self.config['input_shape_emg'] = (num_emg_channels, 4)
                self.config['hidden_sizes_emg'] = [512, 256, 256, 256]
            ...
            if self.config['input_type'] == 'emg+raw_imu':
                ...
                self.config['input_shape_emg'] = (num_emg_channels, 4)
                self.config['input_shape_imu'] = 9
                self.config['hidden_sizes_imu'] = [128, 64, 64]
                self.config['hidden_sizes_emg'] = [128, 64, 64]
                self.model = MyMultimodalNetworkLSTM(input_shape_emg=self.config['input_shape_emg'], input_shape_imu=self.config['input_shape_imu'], 
                                                     num_classes=self.config['num_classes'], hidden_sizes_emg=self.config['hidden_sizes_emg'], 
                ...
                self.config['input_shape_emg'] = (num_emg_channels, 4)
                self.config['hidden_sizes_emg'] = [128, 128, 64]
                # Is not this one the network
            ...
            if self.config['input_type'] == 'emg+raw_imu':
                ...
                self.config['input_shape_emg'] = (num_emg_channels, 4)
                self.config['hidden_sizes_emg'] = [256, 256, 128]
    ...
    # here the label 5 (rest) becomes 0, the other from 1 to 4 remain the same
    ...
    # when we enter in the training mode
    def training_path_callback(self, msg):
        ...
        parts[-1] = "DATASET_ANGLE_ESTIMATION_" + last_char
    ...
    def training_pipeline(self):
        ...
        emg_data, imu_data, label_data, prediction1, prediction2, prediction3 = self.data_manager.load_data('angles_estimation_dataset.npz', directory=self.training_path)
    ...
    def training_pipeline_emg(self):
        print(self.training_path)
        emg_data, _, label_data, prediction1, prediction2, prediction3 = self.data_manager.load_data('emg_dataset.npz', directory=self.training_path)
        print(f"Emg data: {emg_data.shape}, Labels: {label_data.shape}, Predictions: {prediction1.shape}{prediction2.shape}{prediction3.shape}")
        ...
        # Create DataLoaders
        loader1 = DataLoader(TensorDataset(emg_tensor1, label_tensor1), batch_size=self.train_config['batch_size'], shuffle=True)
        loader2 = DataLoader(TensorDataset(emg_tensor2, label_tensor2), batch_size=self.train_config['batch_size'], shuffle=True)
        loader3 = DataLoader(TensorDataset(emg_tensor3, label_tensor3), batch_size=self.train_config['batch_size'], shuffle=True)
        loaders = [loader1, loader2, loader3]
        ...
        for i in range(3):
            ...
            self.model = self.models[i]
            print(f"\n TRAINING{i} is beginning:\n")
            train_EMG(self.model, loaders[i], self.criterion, self.optimizer, self.train_config['epochs'])
            print("\n TRAINING is finished!\n")
            self.models[i] = self.model
        msg = Bool()
        msg.data = False
...
#%% ACQUISITION NODE AND MAIN

# Only node responsible to bridge the communication between Matlab and ROS2, send the window to the pipeline node
# I receive from GUI the current label
class AcquisitionNode(Node):
    def __init__(self):
        ...
        # publisher for sending the effective sampling frequency to the pipeline node
        #self.frequency_pub = self.create_publisher(Float64, '/freq64', qos_profile=rclpy.qos.qos_profile_sensor_data, callback_group=self.callback_group)
        #self.frequency_list = []
        # Just used for clearing the buffers each time the mode changes
        self.mode_sub = self.create_subscription(Int32, '/mode32', self.mode_callback, qos_profile=rclpy.qos.qos_profile_sensor_data, callback_group=self.callback_group)
        # publish the window to the pipeline node
        self.window_pub = self.create_publisher(Float64MultiArray, '/window32', qos_profile=rclpy.qos.qos_profile_sensor_data, callback_group=self.callback_group)
        # timing managing for calculating samplig frequency
    ...
    def mode_callback(self, msg):
        self.window_sender(clear=True)

    def window_sender(self, emg=None, imu=None, clear=False):
        ...
        while len(self.emg_list) >= 2 and len(self.imu_list) >= 2:
            window = np.hstack((self.emg_list.pop(0), self.emg_list[0], self.imu_list.pop(0), self.imu_list[0]))
        ...
        #print(f"Emg list: {len(self.emg_list)}, Imu list: {len(self.imu_list)}")

    def emg_callback(self, msg):
        self.window_sender(emg=np.array(msg.data))

    def imu_callback(self, msg):
        self.window_sender(imu=np.array(msg.data))
...
def main(args=None):
    ...
    executor.add_node(acquisition_node)
